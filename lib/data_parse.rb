  ...
  csv_table.by_row.each do |row|
    unless row["Entry.status"] == "Removed"
      ...
      if data[device_type][manufacturer][model_hardware_version].length > 0
        ...
        data[device_type][manufacturer][model_hardware_version][firmware_version] ||= {
        ...
        data[device_type][manufacturer][model_hardware_version][firmware_version].merge!(inner_hash)
        ...
        puts data
        data[device_type][manufacturer][model_hardware_version].delete_if{|k,v| k.is_a?(Symbol)}
        puts data
